...
def plot_histograms(X: Union[np.ndarray, List[np.ndarray]],
                    ...
                    quantiles: Tuple[float, float]=(0., 1.)) -> plt.figure:
    """
    ...
    - quantiles (Union[None, Tuple[float, float]]): (min, max)-quantile range for display. Bins will be adapted accordingly
    ...
    xmin = combined_data[int(len(combined_data) * quantiles[0])]
    xmax = combined_data[int((len(combined_data) - 1) * quantiles[1])]
    xrange = (xmin, xmax)
    common_bins = np.histogram_bin_edges(combined_data, bins=bins, range=xrange)
    ...
    for i, data in enumerate(X):
        ...
        ax.hist(data, bins=common_bins, alpha=0.5, label=label, color=color, density=density, range=xrange)
